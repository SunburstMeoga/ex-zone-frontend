...
const Trade = () => {
    ...
    const handleApproveAndSwap = async () => { //点击approve and swap
        ...
        try {
            ...
            setTransactionDetails({ token0Used: fromTokenValue, token1Received: toTokenValue, fee: (selectFeeInfo.value) * 0.01 + '%', token0Balance: balanceOne, token1Balance: balanceTwo, token0: fromTokenInfo.title, token1: toTokenInfo.title });
    ...
    const selectTokenItem = async (item) => { //选择token
        ...
        if (toTokenInfo.title && fromTokenInfo.title) {
            console.log('获取交易对价格')
            getTranPrice()
        }
        // return
    ...
    const fetchPrice = async () => { //计算实时价格与兑换比率
        try {
            ...
            // 计算价格
            const price = sqrtPriceX96 ** 2 / (2 ** 192);  // 价格 = (sqrtPriceX96 / 2^96) ^ 2
            // setTransPrice(price)
            ...
            <div className='pt-4-8 lg:pt-6-9 bg-black'>
                <div className='w-full flex flex-col justify-start items-center relative'>
                    ...
                    <div className='w-full flex flex-col justify-start items-center gradient-swap-module h-auto pb-8-0 backdrop-blur-xl text-white lg:min-h-screen relative z-10'>
                            ...
                            <div className='bg-swap-card-module flex flex-col justify-between items-center border-2 border-swap-border rounded-2xl p-0-8 w-20-0 h-8-0 lg:w-35-0'>
                                ...
                                <div className='w-full flex justify-end'>
                                    {isCalculating ? (
                                        ...
                                        <input disabled type="number" step="0.01" value={toTokenValue} className='bg-transparent placeholder-swap-border text-right text-1-2 border-none focus:outline-none caret-swap-border h-3-0 w-full' onChange={handleInputFocus}></input>
                                    )}
                        ...
                        {fromTokenInfo.title && toTokenInfo.title && <div className='w-20-0 mb-2-0 lg:w-35-0 lg:mb-8-0 text-gray-400' >
                            {fromTokenInfo.title} Per {transPrice} {toTokenInfo.title}
                        </div>}
